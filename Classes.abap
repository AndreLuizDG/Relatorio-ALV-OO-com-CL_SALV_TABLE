...
*$*$ Melhorias: Implementar a classe que faz o bot�o de gerar CSV   *$*$
*$*$ t� com problema pq o metodo � privado                          *$*$
*$*$ -------------------------------------------------------------- *$*$
...
CLASS cla_alv IMPLEMENTATION.
  ...
  METHOD validacao_inicial.
    ...
    CALL FUNCTION 'FI_PERIOD_DETERMINE'
      EXPORTING
        i_budat        = s_budat-low "Data inicial da minha tela de sele��o
        i_bukrs        = p_bukrs     "Empresa da minha tela de sele��o
      IMPORTING
        e_gjahr        = v_gjahr_i "Exerc�cio
        e_monat        = v_monat_i "M�s do exerc�cio
    ...
    CALL FUNCTION 'FI_PERIOD_DETERMINE'
      EXPORTING
        i_budat        = s_budat-high "Data inicial da minha tela de sele��o
        i_bukrs        = p_bukrs      "Empresa da minha tela de sele��o
      IMPORTING
        e_gjahr        = v_gjahr_f "Exerc�cio
        e_monat        = v_monat_f "M�s do exerc�cio
    ...
    IF ti_anlp IS NOT INITIAL.
      ...
      CONCATENATE
      'Para a empresa' p_bukrs 'j� foi lan�ada deprecia��o para o per�odo informado.'
      'Ser� necess�rio executar transa��o afab  com a op��o �repeti��o�.'
      'Deseja prosseguir?'
      INTO v_help
       SEPARATED BY space.
...
*    Fazer a valida��o na hora de chamar os metodos

  ENDMETHOD.

  METHOD seleciona_dados.

*   Cabe�alho de documento do lan�amento de imobilizado
    FREE   ti_anek.
    SELECT bukrs
           anln1
           anln2
      ...
      FROM anek
      INTO TABLE  ti_anek.
*     (EST� COMENTANDO PQ N�O TEM DADOS PARA ESSE CENARIO)
*     WHERE bukrs  = p_bukrs   "Empresa
*       AND anln1  = s_anln1   "Imobilizado
*       AND anln2  = s_anln2   "Subn�mero
*       AND budat  = s_budat.    "Data de Lan�amento
    ...
    IF ti_anek IS  NOT INITIAL.
     ...
     FOR ALL ENTRIES IN ti_anek
       WHERE bukrs  =  ti_anek-bukrs  "Empresa
         ...
         AND anln2  =  ti_anek-anln2. "Subn�mero
*        AND anlkl  IN s_anlkl.       "Classe do Imobilizado (EST� COMENTANDO PQ N�O TEM DADOS PARA ESSE CENARIO)
...
*       Cabe�alho de documento do lan�amento de imobilizado
    ...
    LOOP AT ti_anep INTO wa_anep.
      ...
      ELSE.
        ...
        ELSE.
          ...
          wa_saida-anln2 = wa_anep-anln2. "�Subn�mero

          READ TABLE ti_anla INTO wa_anla WITH KEY
                                          bukrs = wa_anep-bukrs "Empresa
                                          anln1 = wa_anep-anln1 "Imobilizado
                                          anln2 = wa_anep-anln2 "Subn�mero
                                          BINARY SEARCH.
          IF sy-subrc = 0.
          ...
          READ TABLE ti_tabwt INTO wa_tabwt WITH KEY
                                            ...
                                            BINARY SEARCH.
          IF sy-subrc = 0.
            wa_saida-bwatxt = wa_tabwt-bwatxt. "Descri��o do tipo de movimento
          ENDIF.
  ...
  METHOD monta_alv.
    ...
    wa_fieldcat-reptext_ddic = 'MONTANTE LAN�ADO'.
